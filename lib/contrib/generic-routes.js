...
var routes = {
    ...
    groupBy: {
        ...
        handler: function handler(request, reply) {
            ...
            var operator = _request$query.operator;
            var target = _request$query.target;
    ...
    aggregate: {
        ...
        handler: function handler(request, reply) {
            var Model = request.Model;
            var db = request.db;
            var _request$pre4 = request.pre;
            var queryFilter = _request$pre4.queryFilter;
            var queryOptions = _request$pre4.queryOptions;
            var label = request.query.label;

            db.aggregate(Model.name, label, queryFilter, queryOptions).then(function (data) {
                return reply.jsonApi({ data: data });
            })['catch'](function (err) {
                if (err.name === 'ValidationError') {
                    return reply.badRequest(err.message, err.extra);
    ...
    stream: {
        ...
        handler: function handler(request, reply) {
            var _request$pre5 = request.pre;
            var queryFilter = _request$pre5.queryFilter;
            var queryOptions = _request$pre5.queryOptions;
            var Model = request.Model;
            var apiBaseUri = request.apiBaseUri;
            var db = request.db;
            var _request$query2 = request.query;
            var delimiter = _request$query2.delimiter;
            var header = _request$query2.header;
