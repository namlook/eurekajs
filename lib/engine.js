...
var buildThumbnail = function(filePath, uploadDirectory, callback) {
    ...
    var thumbPath = filePath.split('.').slice(0, -1).join('.');
    var extension = filePath.split('.').slice('-1');
    thumbPath = thumbPath+'-thumb.'+extension;
...
engine.redirectToFile = function(req, res) {
    ...
    return res.redirect(301, '/uploads/'+type+'/'+file);
};
